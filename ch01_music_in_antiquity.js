...
// ── SLIDE 2 · Chapter Overview ───────────────────────────────────────────────
{
  ...
  sections.forEach(([icon, title, sub], i) => {
    ...
    s.addText(sub, { x: 1.15, y: y + 0.31, w: 8.4, h: 0.28, fontSize: 11.5, color: C.midBrown, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 3 · How Do We Know? (4 types of evidence) ─────────────────────────
{
  ...
  cards.forEach(([num, title, desc], i) => {
    ...
    s.addText(title, { x: x + 0.1, y: 2.15, w: 1.95, h: 0.85, fontSize: 13.5, bold: true, color: C.cream, align: "center", fontFace: "Georgia" });
    s.addShape(pres.ShapeType.rect, { x: x + 0.3, y: 3.05, w: 1.55, h: 0.03, fill: { color: C.gold } });
    s.addText(desc, { x: x + 0.1, y: 3.15, w: 1.95, h: 1.7, fontSize: 11.5, color: C.sand, align: "center", fontFace: "Calibri" });
  });
}
...
// ── SLIDE 4 · The Earliest Music (Prehistoric) ───────────────────────────────
{
  ...
  const bullets = [
    "石器時代：人們在動物骨骼和猛獁象牙上鑽孔製作笛子\n Stone Age: finger-holes bored into animal bones and mammoth ivory",
    "現存最古老骨笛約 40,000–42,000 BCE，出土於德國 Hohle Fels 洞穴（格里芬禿鷲翅骨）\n Oldest bone flute: ~40,000 BCE, Hohle Fels Cave, Germany (griffon vulture radius)",
    "洞穴壁畫疑似描繪樂器演奏；土耳其新石器壁畫（西元前 6000 年）顯示鼓手為舞者伴奏\n Cave paintings may depict instruments; Neolithic Turkey murals show drummers",
    "青銅時代（BC 4000 起）：金屬樂器（鐘、鈸、角）和撥弦樂器出現\n Bronze Age (4th mill. BCE): metal instruments and plucked strings emerge",
    "沒有文字記錄 → 史前音樂幾乎完全沉默\n No written record → prehistoric music is nearly entirely silent",
  ];

  s.addText(bullets.map((b, i) => ({
    text: b, options: { bullet: true, breakLine: i < bullets.length - 1, fontSize: 12.5, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 4 },
  ...
  ], { x: 6.4, y: 1.55, w: 3.1, h: 2.5, fontSize: 12, color: C.sand, fontFace: "Calibri" });

  s.addText("🔑 關鍵觀念 Key Insight", { x: 6.4, y: 4.15, w: 3.1, h: 0.35, fontSize: 12, bold: true, color: C.gold, align: "center" });
  s.addText("文字的發明標誌史前時代的結束，也是音樂歷史的真正起點\nThe invention of writing marks the true beginning of music history.", {
    x: 6.4, y: 4.52, w: 3.1, h: 0.7, fontSize: 10.5, color: C.sand, fontFace: "Calibri", align: "center",
  });
}
...
// ── SLIDE 5 · Mesopotamia Overview ───────────────────────────────────────────
{
  ...
  facts.forEach(([label, content], i) => {
    ...
    s.addText(label, { x: 0.45, y: y + 0.07, w: 2.0, h: 0.55, fontSize: 12.5, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(content, { x: 2.4, y: y + 0.05, w: 7.1, h: 0.62, fontSize: 12, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 6 · Mesopotamian Instruments ───────────────────────────────────────
{
  ...
  s.addText("Mesopotamian Instruments · ca. 2500 BCE, Royal Tombs at Ur", {
    x: 0.4, y: 0.7, w: 9.2, h: 0.3, fontSize: 13, color: C.midBrown, fontFace: "Calibri", italic: true, margin: 0,
  });
  ...
  topLeft.forEach(([name, desc], i) => {
    ...
    s.addText(desc, { x: 0.55, y: y + 0.5, w: 4.2, h: 0.65, fontSize: 11.5, color: C.cream, fontFace: "Calibri", margin: 0 });
  });
  topRight.forEach(([name, desc], i) => {
    ...
    s.addText(desc, { x: 5.25, y: y + 0.5, w: 4.2, h: 0.65, fontSize: 11.5, color: C.cream, fontFace: "Calibri", margin: 0 });
  });
  ...
  // Bull Lyre — featured full-width card at the bottom
  {
    ...
    s.addText(bullDesc, { x: 0.6, y: 4.26, w: 9.0, h: 0.52, fontSize: 11.5, color: C.cream, fontFace: "Calibri", margin: 0 });
  }
  ...
  s.addText("烏爾出土的豎琴（ca. 2500 BCE）是現存最早的撥弦樂器實物之一\nThe lyres and harps from Ur (ca. 2500 BCE) are among the earliest surviving plucked string instruments.", {
    x: 0.4, y: 5.1, w: 9.2, h: 0.35, fontSize: 10.5, color: C.midBrown, italic: true, fontFace: "Calibri",
  });
}
...
// ── SLIDE 7 · First Named Composer: Enheduanna ───────────────────────────────
{
  ...
  const details = [
    ["⏱ 年代", "活躍於 ca. 2300 BCE"],
    ["👩‍⚕️ 身份", "阿卡德族（Akkadian）月神南納（Nanna）的女大祭司，任職於烏爾"],
  ...
  details.forEach(([label, text], i) => {
    const y = 2.7 + i * 0.55;
    s.addText(label + "  ", { x: 0.6, y, w: 1.4, h: 0.45, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia", align: "right", margin: 0 });
    s.addText(text, { x: 2.1, y, w: 7.5, h: 0.45, fontSize: 13, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 8 · Oldest Notation: Hurrian Hymn ──────────────────────────────────
{
  ...
  s.addText(hLines.map((t, i) => ({
    text: t, options: { breakLine: i < hLines.length - 1, fontSize: 12, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2 },
  ...
  ], { x: 6.1, y: 1.98, w: 3.4, h: 3.2, fontSize: 11.5, color: C.cream, fontFace: "Calibri" });
}
...
// ── SLIDE 9 · Timeline ────────────────────────────────────────────────────────
{
  ...
  events.forEach(([date, event], i) => {
    ...
    // Date (left)
    s.addText(date, { x: 0.1, y, w: 2.25, h: 0.33, fontSize: 10, color: C.sand, fontFace: "Calibri", align: "right", margin: 0 });
    // Event (right)
    s.addText(event, { x: 2.9, y, w: 6.8, h: 0.33, fontSize: 10.5, color: C.lightText, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 10 · Music in Ancient Greece — Overview ────────────────────────────
{
  ...
  s.addText("古希臘是現存資料最豐富的古代音樂文明，保存了：\n40 餘件音樂作品或片段 · 大量文字理論著作 · 數百件陶器圖像 · 部分實物樂器", {
    x: 0.4, y: 0.88, w: 9.2, h: 0.7, fontSize: 13, color: C.sand, italic: true, fontFace: "Calibri", align: "center",
  });

  const cols = [
    {
      title: "音樂與宗教 Music & Religion",
      items: [
        "音樂起源被神話化——阿波羅、俄耳甫斯等神祇",
        "Music's origins were mythologized (Apollo, Orpheus)",
        "遍及所有希臘生活面向：工作、軍事、學校、宗教、詩歌、劇場",
        "Pervaded all of Greek life",
      ],
    },
    {
      title: "音樂與詩歌 Music & Poetry",
      items: [
        "「melos」= 旋律、歌詞、舞蹈三合一",
        "Melos: melody + text + stylized dance",
        "希臘人主要是單音音樂（monophonic）",
        "Music primarily monophonic",
        "「lyric poetry」= 配合里拉琴演唱的詩",
        "Lyric poetry = sung to the lyre",
      ],
    },
    {
      title: "音樂與數字 Music & Numbers",
      items: [
        "Pythagoras：八度 = 2:1，五度 = 3:2，四度 = 4:3",
        "Octave 2:1, Fifth 3:2, Fourth 4:3",
        "「harmonia」= 整體秩序、數學比例、音階",
        "Harmonia = mathematical order of universe",
        "「音樂之球」天文學類比",
  ...
  cols.forEach(({ title, items }, i) => {
    ...
    s.addText(title, { x: x + 0.1, y: 1.75, w: 2.85, h: 0.55, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia", align: "center" });
    s.addShape(pres.ShapeType.rect, { x: x + 0.2, y: 2.32, w: 2.65, h: 0.03, fill: { color: C.gold } });
    s.addText(items.map((t, j) => ({
      text: t, options: { bullet: true, breakLine: j < items.length - 1, fontSize: 11, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 3 },
...
// ── SLIDE 11 · Greek Instruments ─────────────────────────────────────────────
{
  ...
  const instruments = [
    ...
    },
    {
      ...
      desc: [
        "大型里拉琴，通常站立演奏",
        "Larger lyre, normally played standing",
        "用於遊行、神聖儀式和劇場",
        "Processions, sacred ceremonies, theater",
        "「基薩拉歌手」（kitharode）= 邊彈邊唱",
        "Kitharode = singer accompanying himself",
      ],
    },
  ];
  ...
  instruments.forEach(({ name, zh, god, desc }, i) => {
    const x = 0.35 + i * 3.2;
    s.addShape(pres.ShapeType.rect, { x, y: 0.82, w: 3.0, h: 4.55, fill: { color: i === 1 ? C.wine : C.rust }, rounding: true });
    s.addText(name, { x: x + 0.1, y: 0.92, w: 2.8, h: 0.45, fontSize: 15, bold: true, color: C.gold, fontFace: "Georgia", align: "center" });
    s.addText(zh, { x: x + 0.1, y: 1.37, w: 2.8, h: 0.35, fontSize: 13, color: C.cream, fontFace: "Georgia", align: "center" });
    s.addText(god, { x: x + 0.1, y: 1.72, w: 2.8, h: 0.32, fontSize: 10.5, color: C.sand, fontFace: "Calibri", align: "center", italic: true });
    s.addShape(pres.ShapeType.rect, { x: x + 0.25, y: 2.07, w: 2.5, h: 0.03, fill: { color: C.gold } });
    s.addText(desc.map((t, j) => ({
      text: t, options: { bullet: true, breakLine: j < desc.length - 1, fontSize: 10.5, color: C.cream, fontFace: "Calibri", paraSpaceAfter: 2 },
    })), { x: x + 0.1, y: 2.13, w: 2.8, h: 3.15 });
  });

  s.addText("希臘人主要靠耳朵學習音樂，大多從記憶或即興演奏，而非讀譜\nGreeks primarily learned by ear — from memory or improvisation, not reading notation", {
    x: 0.4, y: 5.15, w: 9.2, h: 0.35, fontSize: 10.5, color: C.midBrown, italic: true, fontFace: "Calibri",
  });
}
...
// ── SLIDE 12 · IN PERFORMANCE: Competitions ──────────────────────────────────
{
  ...
  s.addText("IN PERFORMANCE", { x: 0.4, y: 0.2, w: 3.2, h: 0.45, fontSize: 13, bold: true, color: C.darkBg, align: "center", fontFace: "Georgia" });
  ...
  items.forEach(([label, text], i) => {
    ...
    s.addText(label, { x: 0.45, y: y + 0.07, w: 2.2, h: 0.5, fontSize: 12.5, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(text, { x: 2.7, y: y + 0.05, w: 6.9, h: 0.55, fontSize: 11.5, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 13 · Music and Ethos ────────────────────────────────────────────────
{
  ...
  s.addText("希臘哲學家相信音樂能直接影響人的性格與道德 · Greek philosophers believed music could affect ethical character", {
    x: 0.4, y: 0.7, w: 9.2, h: 0.35, fontSize: 12.5, color: C.midBrown, italic: true, fontFace: "Calibri", margin: 0,
  });
  ...
  const philos = [
    ...
    },
    {
      ...
      color: C.rust,
      points: [
        "音樂能模仿並因此激發人的ethos（性格狀態）",
        "Music imitates and thus arouses an ethos in the listener",
        "態度比柏拉圖更開放：音樂亦可用於娛樂",
        "More open than Plato: music can serve enjoyment too",
        "負面情緒（恐懼、悲憫）可藉音樂宣洩（catharsis）",
        "Negative emotions purged through music — catharsis",
        "Mixolydian → 悲傷；Dorian → 平靜；Phrygian → 激昂",
        "Mixolydian = mournful; Dorian = composed; Phrygian = ecstatic",
      ],
    },
  ];
  ...
  philos.forEach(({ name, years, book, color, points }, i) => {
    ...
    s.addText(`${years} · ${book}`, { x: x + 0.1, y: 1.7, w: 4.3, h: 0.32, fontSize: 11, color: C.sand, align: "center", italic: true, fontFace: "Calibri" });
    s.addShape(pres.ShapeType.rect, { x: x + 0.3, y: 2.05, w: 3.9, h: 0.03, fill: { color: C.gold } });
    s.addText(points.map((t, j) => ({
      text: t, options: { bullet: true, breakLine: j < points.length - 1, fontSize: 11, color: C.cream, fontFace: "Calibri", paraSpaceAfter: 2 },
...
// ── SLIDE 14 · Greek Music Theory ─────────────────────────────────────────────
{
  ...
  concepts.forEach(({ term, def, eng, extra }, i) => {
    ...
    s.addText(def, { x: 0.6, y: y + 0.45, w: 4.4, h: 0.32, fontSize: 11.5, color: C.darkText, fontFace: "Calibri", margin: 0 });
    s.addShape(pres.ShapeType.rect, { x: 5.1, y: y + 0.08, w: 0.03, h: 0.88, fill: { color: C.sand } });
    s.addText(eng, { x: 5.2, y: y + 0.07, w: 4.2, h: 0.38, fontSize: 11.5, color: C.midBrown, italic: true, fontFace: "Calibri", margin: 0 });
    s.addText(extra, { x: 5.2, y: y + 0.5, w: 4.2, h: 0.47, fontSize: 10.5, color: C.slate, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 15 · Surviving Greek Music: Seikilos ────────────────────────────────
{
  ...
  ], { x: 0.4, y: 0.82, w: 9.2, h: 0.55, fontSize: 13, color: C.sand, fontFace: "Calibri" });
  ...
  s.addText("現存最古老完整的音樂作品 · ca. 1st century CE · 出土於土耳其特拉勒斯（Tralles）", {
    x: 0.5, y: 2.05, w: 9, h: 0.35, fontSize: 12, color: C.sand, align: "center", italic: true, fontFace: "Calibri",
  });
  ...
  s.addText([
    ...
    { text: "趁著活著，閃耀吧；莫讓任何事令你憂愁。\n生命短暫，時間終會把它帶走。\n\n", options: { fontSize: 13, color: C.cream } },
    { text: "While you live, shine; let nothing grieve you.\nLife is short, and time claims its toll.", options: { fontSize: 13, color: C.sand, italic: true } },
...
// ── SLIDE 16 · Music in Ancient Rome ─────────────────────────────────────────
{
  ...
  leftItems.forEach(([label, content], i) => {
    ...
    s.addText(label, { x: 0.55, y: y + 0.07, w: 5.3, h: 0.35, fontSize: 13.5, bold: true, color: C.rust, fontFace: "Georgia", margin: 0 });
    s.addText(content, { x: 0.55, y: y + 0.43, w: 5.3, h: 0.58, fontSize: 11.5, color: C.darkText, fontFace: "Calibri", margin: 0 });
  });
  ...
  s.addText(romInstr.map((t, i) => ({
    text: t, options: { bullet: true, breakLine: i < romInstr.length - 1, fontSize: 12, color: C.cream, fontFace: "Calibri", paraSpaceAfter: 3 },
...
// ── SLIDE 17 · The Greek Heritage ─────────────────────────────────────────────
{
  ...
  legacies.forEach(([label, text], i) => {
    ...
    s.addText(label, { x: 0.45, y: y + 0.08, w: 2.1, h: 0.55, fontSize: 12.5, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(text, { x: 2.6, y: y + 0.05, w: 7.0, h: 0.62, fontSize: 12, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 18 · Summary ─────────────────────────────────────────────────────────
{
  ...
  s.addText(summaryPoints.map((t, i) => ({
    text: t, options: { bullet: true, breakLine: i < summaryPoints.length - 1, fontSize: 12, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 5 },
...
// ── SLIDE 19 · 延伸閱讀 Further Reading ──────────────────────────────────────
{
  ...
  resources.forEach(({ cat, items }, i) => {
    const y = 1.1 + i * 1.52;
    s.addText(cat, { x: 0.4, y, w: 9.2, h: 0.38, fontSize: 14.5, bold: true, color: C.wine, fontFace: "Georgia", margin: 0 });
    s.addText(items.map((t, j) => ({
      text: t, options: { bullet: true, breakLine: j < items.length - 1, fontSize: 11, color: C.slate, fontFace: "Calibri", paraSpaceAfter: 2 },
    })), { x: 0.6, y: y + 0.38, w: 9.0, h: 1.1 });
  });
}
