...
// ── SLIDE 2 · Chapter Overview ───────────────────────────────────────────────
{
  ...
  s.addText("本章概覽 Chapter Overview", { x: 0.4, y: 0.2, w: 9.2, h: 0.6, fontSize: 26, bold: true, color: C.teal, fontFace: "Georgia" });
  ...
  sections.forEach(([icon, title, sub], i) => {
    ...
    s.addText(sub, { x: 1.15, y: y + 0.28, w: 8.4, h: 0.26, fontSize: 11, color: C.midBlue, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 3 · Europe 1400–1600 ───────────────────────────────────────────────
{
  ...
  s.addText("Europe from 1400 to 1600 · Political, Economic, and Technological Change", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });

  // Left: Political/Religious
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 4.6, h: 3.95, fill: { color: "2A3C50" }, rounding: true });
  s.addText("⚔ 政治與宗教 Political & Religious", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  ...
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "2A3C50" }, rounding: true });
  s.addText("💰 經濟與技術 Economic & Technological", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("• ca. 1400 歐洲經濟復甦 · 區域專業化與長距離貿易\n• 中產階級（商人、工匠、律師）興起\n• 1450 Gutenberg 活字印刷術\n• 1487 葡萄牙繞過非洲南端\n• 1492 Columbus 抵達西印度群島\n• 歐洲殖民擴張至美洲、非洲、亞洲\n• 科學、造船、火炮、航海技術突破\n• 義大利城邦的宮廷贊助成為藝術發展的沃土", {
    x: 5.3, y: 1.72, w: 4.35, h: 3.45, fontSize: 9.5, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 3,
  });
}
...
// ── SLIDE 4 · Humanism ───────────────────────────────────────────────────────
{
  ...
  s.addText("studia humanitatis · The Revival of Ancient Learning", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  ...
  s.addText("📖 核心理念 Core Ideas", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("• studia humanitatis：文法、修辭、詩學、歷史、道德哲學\n• 反對經院哲學的邏輯與權威——轉而重視個人道德、公民生活\n• 古典希臘與羅馬文獻的廣泛取得（拜占庭學者西遷）\n• 對人類尊嚴、理性、感官經驗的信心\n• 與基督教融合——而非取代\n• 重塑大學課程，影響藝術家、音樂家、政治家", {
    x: 0.5, y: 1.72, w: 4.35, h: 3.45, fontSize: 9.5, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 3,
  });

  // Effects on music
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("🎵 對音樂的影響 Effects on Music", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("• 古代音樂論著的再發現（Aristides Quintilianus、Ptolemy、Cleonides）\n• Aristotle、Plato、Quintilian 視音樂為公民教育必備\n• 修辭學思維進入作曲——結構、說服、清晰度\n• 對詞曲關係的新重視——自然音節與意義\n• 音樂被視為「有表情的語言」\n• 音樂作為紳士仕女社交禮儀的一部分（彈琵琶而非吹笛）\n• Franchino Gaffurio（1451–1522）融合古代理論與當代實踐", {
    x: 5.3, y: 1.72, w: 4.35, h: 3.45, fontSize: 9.5, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 3,
  });
}
...
// ── SLIDE 5 · Renaissance in Art ─────────────────────────────────────────────
{
  ...
  s.addText("Sculpture, Painting, Architecture · Parallels with Music", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });
  ...
  s.addText("藝術元素", { x: 0.4, y: 1.32, w: 2.9, h: 0.32, fontSize: 11, bold: true, color: C.darkText, fontFace: "Georgia", align: "center" });
  s.addText("藝術中的表現 Artistic Expression", { x: 3.3, y: 1.32, w: 3.4, h: 0.32, fontSize: 11, bold: true, color: C.darkText, fontFace: "Georgia", align: "center" });
  s.addText("音樂中的對應 Musical Parallel", { x: 6.7, y: 1.32, w: 3.0, h: 0.32, fontSize: 11, bold: true, color: C.darkText, fontFace: "Georgia", align: "center" });
  ...
  artRows.forEach(([k, a, m], i) => {
    ...
    s.addText(k, { x: 0.4, y: y + 0.04, w: 2.9, h: 0.52, fontSize: 9.5, bold: true, color: C.gold, fontFace: "Georgia" });
    s.addText(a, { x: 3.3, y: y + 0.04, w: 3.4, h: 0.52, fontSize: 8.5, color: C.sand, fontFace: "Calibri" });
    s.addText(m, { x: 6.7, y: y + 0.04, w: 3.0, h: 0.52, fontSize: 8.5, color: C.sand, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 6 · Patronage and Training ─────────────────────────────────────────
{
  ...
  s.addText("Patronage and the Training of Musicians", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  ...
  divs.forEach(([icon, title, desc], i) => {
    ...
    s.addText(title, { x: x + 0.7, y: 1.35, w: 2.25, h: 0.3, fontSize: 12, bold: true, color: C.teal, fontFace: "Georgia" });
    s.addText(desc, { x: x + 0.12, y: 1.75, w: 2.78, h: 1.3, fontSize: 9, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2 });
  });

  // Training
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 3.2, w: 9.4, h: 2.1, fill: { color: "E0EEEC" }, rounding: true });
  s.addText("🎓 音樂家的養成 Training and Mobility", { x: 0.5, y: 3.3, w: 9.0, h: 0.35, fontSize: 13, bold: true, color: C.teal, fontFace: "Georgia" });
  s.addText("• 歌手：主教座堂與教堂的唱詩學校——學唱歌、樂理、調式、對位、作曲、拉丁文、神學；Cambrai、Bruges、Antwerp、Paris 為 15 世紀北方音樂重鎮\n• 器樂手：音樂家族傳承、工會師徒制；多數不識譜——音樂口傳至 16 世紀印刷術興起\n• Benefices（教會俸祿）：教宗授予神職人員的薪水制度——作曲家大多為神職；女性與已婚男性無法受惠\n• 國際流動：Du Fay、Josquin、Ockeghem、Obrecht、Willaert 等法蘭德斯人遍布義大利；Medici、Este、Sforza、Gonzaga 家族競相延攬\n• Johannes Ciconia（ca. 1370–1412）最早赴義大利發展的北方作曲家之一", {
    x: 0.5, y: 3.65, w: 9.0, h: 1.6, fontSize: 9, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 3,
  });
}
...
// ── SLIDE 7 · The New Counterpoint ───────────────────────────────────────────
{
  ...
  s.addText("The Core of the International Style · ca. 1420 onward", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });
  ...
  principles.forEach(([icon, title, desc], i) => {
    ...
    s.addText(title, { x: 1.1, y: y + 0.05, w: 8.5, h: 0.32, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia" });
    s.addText(desc, { x: 1.1, y: y + 0.35, w: 8.5, h: 0.3, fontSize: 10, color: C.sand, fontFace: "Calibri" });
  });

  // Tinctoris quote
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 4.45, w: 9.4, h: 0.78, fill: { color: "3E2030" }, rounding: true });
  s.addText("Johannes Tinctoris (ca. 1435–1511) · Liber de arte contrapuncti (1477)", { x: 0.45, y: 4.5, w: 9.1, h: 0.3, fontSize: 10, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("「40 年前的作品沒有一首值得演奏……只有 Dunstable、Binchois、Du Fay 與 Ockeghem 這一輩的音樂才值得模仿。」", { x: 0.45, y: 4.78, w: 9.1, h: 0.42, fontSize: 10, color: C.sand, fontFace: "Calibri" });
}
...
// ── SLIDE 8 · New Textures ───────────────────────────────────────────────────
{
  ...
  s.addText("Imitative Counterpoint & Homophony · Dominant in the 16th Century", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.12, w: 5, h: 0.04, fill: { color: C.wine } });

  // Imitation
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 4.6, h: 3.95, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("🔁 Imitative Counterpoint 模仿對位", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("定義：一聲部提出動機或樂句，其他聲部在不同音高（通常五度、四度或八度）回應模仿", {
    x: 0.5, y: 1.72, w: 4.35, h: 0.7, fontSize: 9.5, color: C.darkText, fontFace: "Calibri",
  });
  s.addText("• 所有聲部平等參與——無 cantus/tenor 主從之分\n• 每個樂段通常有一個音樂動機（motto）\n• 文字宣告清晰——所有聲部同節奏唱出相同字句\n• 容許結構複雜而仍然清楚\n• 成為 16 世紀經文歌、彌撒、器樂作品的核心技法\n• 集大成者：Josquin、Lassus、Palestrina", {
    x: 0.5, y: 2.5, w: 4.35, h: 2.7, fontSize: 9, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 3,
  });

  // Homophony
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("♫ Homophony 主調織度", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("定義：所有聲部以相同（或近乎相同）節奏進行——低聲部以協和音伴奏 cantus", {
    x: 5.3, y: 1.72, w: 4.35, h: 0.7, fontSize: 9.5, color: C.darkText, fontFace: "Calibri",
  });
  s.addText("• 結構簡單直接——容易演唱、聆聽\n• 文字宣告極為清晰——每字每音一目瞭然\n• 16 世紀通俗歌曲（frottola、villancico、chanson）的主要風格\n• 與模仿對位常交替使用於同一作品中——創造對比\n• 展現 Renaissance 對詞曲關係的新重視\n• 後來發展為主旋律伴奏（monody）——通往 Baroque", {
    x: 5.3, y: 2.5, w: 4.35, h: 2.7, fontSize: 9, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 3,
  });
}
...
// ── SLIDE 9 · Tuning and Temperament ─────────────────────────────────────────
{
  ...
  s.addText("The Challenge of Thirds and Sixths", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });
  ...
  tunings.forEach(([name, when, desc], i) => {
    ...
    s.addText(name, { x: 0.5, y: y + 0.05, w: 4.0, h: 0.32, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia" });
    s.addText(when, { x: 4.5, y: y + 0.08, w: 5.0, h: 0.28, fontSize: 10, color: C.sand, italic: true, fontFace: "Georgia" });
    s.addText(desc, { x: 0.5, y: y + 0.38, w: 9.0, h: 0.42, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 10 · Words and Music ───────────────────────────────────────────────
{
  ...
  s.addText("Text Declamation, Emotion, and the Influence of Ancient Writings", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.12, w: 5, h: 0.04, fill: { color: C.teal } });
  ...
  topics.forEach(([title, desc], i) => {
    ...
    s.addText(title, { x: x + 0.15, y: y + 0.1, w: 4.35, h: 0.35, fontSize: 12, bold: true, color: C.teal, fontFace: "Georgia" });
    s.addText(desc, { x: x + 0.15, y: y + 0.5, w: 4.35, h: 1.35, fontSize: 9, color: C.darkText, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 11 · Music Printing ────────────────────────────────────────────────
{
  ...
  s.addText("Petrucci · Attaingnant · The Technological Revolution", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });

  // Timeline of printing developments
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 9.4, h: 1.85, fill: { color: "2A3C50" }, rounding: true });
  s.addText("🖨 印刷技術的發展 Development of Printing Technology", { x: 0.45, y: 1.38, w: 9.1, h: 0.32, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("• ca. 1450：Johann Gutenberg 在歐洲完善活字印刷術\n• 1470s：活字首次用於音樂——禮拜用書的 chant 記譜\n• 1501：Ottaviano Petrucci（1466–1539）在威尼斯出版 Harmonice musices odhecaton A——第一本完全用活字印刷的複音樂譜（96 首作品）\n  三次壓印：譜線、文字、音符與花體字母分三次印刷——費時但精美\n• ca. 1520：John Rastell 在倫敦首創「單次壓印」\n• 1528：Pierre Attaingnant（ca. 1494–1551/52）在巴黎大規模使用單次壓印——譜線斷續但便宜實用——成為後續標準", {
    x: 0.5, y: 1.72, w: 9.0, h: 1.4, fontSize: 9, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 2,
  });

  // Impact
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 3.25, w: 9.4, h: 2.0, fill: { color: "2A3C50" }, rounding: true });
  s.addText("💡 印刷術的衝擊 Impact of Music Printing", { x: 0.45, y: 3.33, w: 9.1, h: 0.32, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("• 樂譜價格大幅降低——業餘愛好者可負擔\n• 音樂識讀能力普及——中產階級家庭成為音樂活動場域\n• 作曲家的名聲可跨越地域傳播——音樂作為「作品」概念成形\n• 新音樂類型蓬勃發展：國家語言的 madrigal、frottola、chanson、villancico、lute song\n• 記譜器樂音樂首次大量問世：variations、prelude、toccata、ricercare、canzona、sonata\n• 16 世紀末印刷中心：Venice、Paris、Rome、Nuremberg、Lyons、Louvain、Antwerp、London\n• 宗教改革借助印刷——Protestant 與 Catholic 新音樂類型（chorale、anthem、metrical psalm）快速流傳", {
    x: 0.5, y: 3.65, w: 9.0, h: 1.55, fontSize: 9, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 2,
  });
}
...
// ── SLIDE 12 · Key Theorists & Figures ───────────────────────────────────────
{
  ...
  figures.forEach(([name, dates, desc], i) => {
    ...
    s.addText(name, { x: x + 0.15, y: y + 0.05, w: 3.4, h: 0.3, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
    s.addText(dates, { x: x + 0.15, y: y + 0.33, w: 4.3, h: 0.22, fontSize: 9, color: C.slate, fontFace: "Georgia" });
    s.addText(desc, { x: x + 0.15, y: y + 0.53, w: 4.3, h: 0.42, fontSize: 8.5, color: C.darkText, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 13 · Timeline ──────────────────────────────────────────────────────
{
  ...
  s.addText("Europe and Music 1400–1600", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });
...
// ── SLIDE 14 · Legacy / Summary ──────────────────────────────────────────────
{
  ...
  s.addText("Chapter Summary", { x: 0.4, y: 0.75, w: 9.2, h: 0.35, fontSize: 13, italic: true, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.12, w: 5, h: 0.04, fill: { color: C.teal } });
...
// ── SLIDE 15 · Key Terms & Further Reading ───────────────────────────────────
{
  ...
  const termsRight = [
    "Pythagorean · Just · Mean-tone · Equal temperament",
  ...
  termsLeft.forEach((t, i) => {
    s.addText("• " + t, { x: 0.5, y: 1.7 + i * 0.3, w: 4.5, h: 0.28, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });
  termsRight.forEach((t, i) => {
    s.addText("• " + t, { x: 5.1, y: 1.7 + i * 0.3, w: 4.5, h: 0.28, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });

  // Listening (new)
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 3.7, w: 9.4, h: 1.0, fill: { color: "3A2015" }, rounding: true });
  s.addText("🎧 聆聽 Listen (文藝復興代表曲)", { x: 0.45, y: 3.76, w: 9.1, h: 0.3, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText(
    ...
    { x: 0.5, y: 4.04, w: 9.1, h: 0.65, fontSize: 9.5, color: C.sand, fontFace: "Calibri" }
  );

  // Further reading
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 4.8, w: 9.4, h: 0.9, fill: { color: "3E2030" }, rounding: true });
  s.addText("📚 延伸閱讀 Further Reading", { x: 0.45, y: 4.85, w: 9.1, h: 0.28, fontSize: 11, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("Leeman Perkins, Music in the Age of the Renaissance (1999) · Allan Atlas, Renaissance Music (1998)\nReinhard Strohm, The Rise of European Music 1380–1500 (1993) · Tinctoris, The Art of Counterpoint (trans. Seay)", {
    x: 0.5, y: 5.1, w: 9.1, h: 0.55, fontSize: 8, color: C.sand, fontFace: "Calibri", italic: true,
  });
}
