...
// ── SLIDE 1 · Title ──────────────────────────────────────────────────────────
{
  ...
  s.addText("Contenance angloise · Dunstable · Binchois · Du Fay · The Polyphonic Mass", {
    x: 0.4, y: 3.8, w: 9.2, h: 0.4, fontSize: 13, color: C.sand, italic: true, align: "center", fontFace: "Georgia",
  });
...
// ── SLIDE 2 · Chapter Overview ───────────────────────────────────────────────
{
  ...
  sections.forEach(([icon, title, sub], i) => {
    ...
    s.addText(sub, { x: 1.15, y: y + 0.28, w: 8.4, h: 0.26, fontSize: 11, color: C.midGreen, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 3 · English Music & Faburden ───────────────────────────────────────
{
  ...
  s.addText("English Music · Sarum Rite · Faburden · Carol", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });

  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 4.6, h: 3.95, fill: { color: "2A3E2E" }, rounding: true });
  s.addText("🎶 English Sound 英式音響", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  ...
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "2A3E2E" }, rounding: true });
  s.addText("📜 Faburden & Carol", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
...
// ── SLIDE 4 · John Dunstable ─────────────────────────────────────────────────
{
  ...
  s.addText("ca. 1390–1453 · 15 世紀最傑出的英格蘭作曲家", { x: 0.4, y: 0.78, w: 9.2, h: 0.35, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.15, w: 5, h: 0.04, fill: { color: C.wine } });

  // Bio
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 4.6, h: 3.95, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("📖 生平 Biography", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  ...
  // Works
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("🎼 主要作品 Major Works", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("總計約 60 部作品——涵蓋當時所有複音類型：\n\n• 多達 3 部 polyphonic mass cycles\n• 2 組 Gloria-Credo 配對\n• 15 部其他 Mass Ordinary 樂章\n• 12 部 isorhythmic motets\n• 6 部 plainchant settings\n• 20 部其他拉丁聖樂\n• 5 首世俗歌曲\n\n代表作：\n• Quam pulchra es (NAWM 33)\n  自由創作、文字宣告清晰、自然節奏\n• Regina caeli laetare\n  cantus 中使用 paraphrase——chant 旋律華麗化", {
...
// ── SLIDE 5 · Contenance Angloise ────────────────────────────────────────────
{
  ...
  s.addText("Martin Le Franc · Le champion des dames (1440–42)", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });

  // Quote box
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 9.4, h: 1.7, fill: { color: "3A2015" }, rounding: true });
  s.addText("📜 Le Franc 原詩意譯（1440–42）", { x: 0.45, y: 1.38, w: 9.1, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("「Du Fay 與 Binchois 有一種新的實踐方式——在高音與低音中\n創造活潑的協和。他們採取了英格蘭人的風韻（contenance angloise），\n並追隨 Dunstable——這賦予他們的音樂奇妙的愉悅感。」", {
    x: 0.5, y: 1.75, w: 9.0, h: 1.25, fontSize: 11, color: C.sand, italic: true, fontFace: "Georgia", align: "center",
  });

  // What it means
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 3.1, w: 9.4, h: 2.15, fill: { color: "2A3E2E" }, rounding: true });
  s.addText("🎵 英式風韻的音樂特徵 Musical Features", { x: 0.45, y: 3.18, w: 9.1, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("• 頻繁使用和聲三度與六度——常以平行進行呈現\n• 徹底的協和——極少不協和（僅允許嚴格控制的經過音與留音）\n• 避免平行五度與八度（中世紀複音常見）\n• 同節奏織度（homorhythmic）——所有聲部節奏相似\n• 簡單、優美、可歌的旋律線條\n• 清晰的音節宣告 · 規則樂句結構\n• 這些特徵融入 Binchois、Du Fay 的作品中——成為 15 世紀國際風格的核心\n  These traits became the foundation of the mid-15th century international style", {
    x: 0.5, y: 3.52, w: 9.0, h: 1.7, fontSize: 9, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 2,
  });
}
...
// ── SLIDE 6 · Burgundian Court ───────────────────────────────────────────────
{
  ...
  s.addText("Europe's Foremost Center of Music · ca. 1400–1477", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.12, w: 5, h: 0.04, fill: { color: C.forest } });

  // Duchy info
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 4.6, h: 3.95, fill: { color: "E0EEDA" }, rounding: true });
  s.addText("🗺 The Duchy 公國領地", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.forest, fontFace: "Georgia" });
  s.addText("• 名義上為法王的封臣，實際幾乎與法王平起平坐\n• 原始封地：Burgundy（東法國）\n• 擴張至：荷蘭、比利時、東北法國、盧森堡、洛林\n• 首都名義上是 Dijon，但 15 世紀中期重心在 Lille、Bruges、Ghent、Brussels\n• 1419–35 與英格蘭結盟對抗法王\n• 1477 Charles the Bold 戰死——法國吸收勃艮第公國，低地諸省由其女 Mary 繼承\n\n四位主要公爵：\n• Philip the Bold（r. 1363–1404）建立禮拜堂\n• John the Fearless（r. 1404–19）\n• Philip the Good（r. 1419–67）全盛期\n• Charles the Bold（r. 1467–77）業餘音樂家兼作曲家", {
    x: 0.5, y: 1.72, w: 4.35, h: 3.45, fontSize: 8.5, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2,
  });

  // Chapel info
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "E0EEDA" }, rounding: true });
  s.addText("⛪ The Chapel 禮拜堂樂團", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.forest, fontFace: "Georgia" });
  s.addText("• 1384 Philip the Bold 建立\n• 1445 Philip the Good 時期達 23 位歌手——歐洲僅次於英王\n• 音樂家主要來自法蘭德斯與低地諸省\n• Philip the Good 另維持一支樂手隊——小號、鼓、vielle、琵琶、豎琴、風琴、風笛、shawm——來自法、義、德、葡\n\n四種主要作品類型：\n• Chansons（法文世俗歌曲）\n• Motets（經文歌）\n• Magnificats（頌歌）\n• Mass Ordinary settings（彌撒樂章）\n\n多數作品為三聲部——類似 14 世紀法國 chanson，但各聲部音域略廣\n主旋律在 cantus · 對位支援在 tenor · 和聲填充在 contratenor", {
    x: 5.3, y: 1.72, w: 4.35, h: 3.45, fontSize: 8.5, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2,
  });
}
...
// ── SLIDE 7 · Gilles Binchois ────────────────────────────────────────────────
{
  ...
  s.addText("ca. 1400–1460 · 勃艮第宮廷 chanson 大師", { x: 0.4, y: 0.78, w: 9.2, h: 0.35, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.15, w: 5, h: 0.04, fill: { color: C.gold } });

  // Bio
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 4.6, h: 3.95, fill: { color: "2A3E2E" }, rounding: true });
  s.addText("📖 生平 Biography", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("• 生於 Mons——受訓為唱詩班男童與風琴手\n• 1423 遷居 Lille\n• 曾在英王軍隊佔領法國時服侍 William Pole, earl of Suffolk——直接接觸英格蘭音樂\n• 1427 加入 Philip the Good 的禮拜堂\n• 在勃艮第宮廷服侍三十餘年，1453 以豐厚年金退休\n• 主要工作為演出——作曲為次要\n• 與 Dunstable、Du Fay 並列 15 世紀三大作曲家\n\n主要作品：\n• 28 部彌撒樂章（部分為 Gloria-Credo 或 Sanctus-Agnus 配對）\n• 6 部 Magnificats\n• 29 部 motets\n• 51 首 rondeaux\n• 7 首 ballades", {
    x: 0.5, y: 1.72, w: 4.35, h: 3.45, fontSize: 8.5, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 2,
  });

  // De plus en plus
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "2A3E2E" }, rounding: true });
  s.addText("🎵 De plus en plus (NAWM 34, ca. 1425)", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText("形式：rondeau（ABaAabAB）——15 世紀最常見的 chanson 形式\n\n風格混合：\n• 6/8 拍 · hemiola 交叉節奏\n• cantus 宣告清晰——以音節對音為主\n• 主旋律由級進與跳進交織——outline 上行三和弦後級進下行\n• tenor 節奏較慢但旋律同樣優美\n• contratenor 填補和聲——忽上忽下於 tenor\n• 大量三度與六度——幾乎完全協和\n\n終止式：\n• 傳統的 major 6th → octave\n• 新式：contratenor 在倒數第二音下方五度，然後躍升一個八度到倒數音上方五度——現代耳朵聽來像屬到主的進行", {
...
// ── SLIDE 8 · Guillaume Du Fay Biography ─────────────────────────────────────
{
  ...
  s.addText("ca. 1397–1474 · 國際風格的集大成者 · 時代最富盛名的作曲家", { x: 0.4, y: 0.78, w: 9.2, h: 0.35, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.15, w: 5, h: 0.04, fill: { color: C.wine } });

  // Career timeline
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 9.4, h: 2.35, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("🗓 生涯軌跡 Career Timeline", { x: 0.45, y: 1.38, w: 9.1, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("• 生於布魯塞爾附近（神父之子，非婚生）· 1409 進入 Cambrai 主教座堂唱詩班\n• 1414–18 陪同贊助人出席康士坦斯公會議——結束大分裂\n• 1420 服侍義大利 Rimini 的 Malatesta 家族\n• 1426–28 服侍 Bologna 的 Aleman 樞機主教——晉鐸為神父\n• 1428–33 與 1435–37 兩度服務於教宗禮拜堂（羅馬、佛羅倫斯、波隆那）\n• 1433–35 與 1437–39 擔任 Savoy 宮廷首席禮拜堂長（Amadeus VIII）\n• 1439 教會會議廢黜教宗並選舉 Amadeus——Du Fay 為避免雙贊助人衝突退回 Cambrai\n• 1439–52 在 Cambrai（當時由勃艮第統治）擔任行政職\n• 1452–58 再度赴 Savoy · 晚年終老於 Cambrai——獨居富有\n• 作品保存於約 100 份手稿中——從西班牙到波蘭、義大利到蘇格蘭", {
    x: 0.5, y: 1.72, w: 9.0, h: 1.9, fontSize: 9, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2,
  });

  // Major works
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 3.75, w: 9.4, h: 1.5, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("🎼 主要作品 Major Works", { x: 0.45, y: 3.83, w: 9.1, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("• 至少 6 部 masses · 35 部其他彌撒樂章 · 4 部 Magnificats\n• 60 首聖詩與其他聖歌設定 · 24 部 motets（13 首 isorhythmic, 11 首自由創作）· 34 首 plainchant 旋律\n• 60 首 rondeaux · 8 首 ballades · 13 首其他世俗歌曲\n• 代表作：Resvellies vous (NAWM 35)、Christe redemptor omnium (NAWM 36)、Se la face ay pale 與 Missa Se la face ay pale (NAWM 37)、Nuper rosarum flores", {
    x: 0.5, y: 4.15, w: 9.0, h: 1.05, fontSize: 8.5, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2,
  });
}
...
// ── SLIDE 9 · Du Fay Works ───────────────────────────────────────────────────
{
  ...
  s.addText("Chansons · Motets · Fauxbourdon · Nuper rosarum flores", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });
  ...
  works.forEach(([title, desc], i) => {
    ...
    s.addText(title, { x: 0.55, y: y + 0.05, w: 9.1, h: 0.28, fontSize: 11, bold: true, color: C.gold, fontFace: "Georgia" });
    s.addText(desc, { x: 0.55, y: y + 0.3, w: 9.1, h: 0.28, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 10 · Fauxbourdon ───────────────────────────────────────────────────
{
  ...
  s.addText("Continental Adaptation of English Faburden", { x: 0.4, y: 0.78, w: 9.2, h: 0.35, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  ...
  s.addText("🎼 技法說明 The Technique", { x: 0.45, y: 1.38, w: 9.1, h: 0.32, fontSize: 12, bold: true, color: C.forest, fontFace: "Georgia" });
  s.addText("• 只寫出兩個聲部：cantus 與 tenor——主要以平行六度進行\n• 第三聲部不寫出——由歌手自動唱在 cantus 下方精確的平行四度\n• 結果形成連續的 6/3 和弦（類似英格蘭 faburden）\n• 每樂句末終止於開放五度與八度\n• 受英格蘭 faburden 啟發——但技法不同（faburden 中聲部持 chant）", {
    x: 0.5, y: 1.72, w: 9.0, h: 1.25, fontSize: 9.5, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 3,
  });

  // Usage & comparison
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 3.1, w: 4.6, h: 2.15, fill: { color: "E0EEDA" }, rounding: true });
  s.addText("📚 用途 Usage", { x: 0.45, y: 3.18, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.forest, fontFace: "Georgia" });
  s.addText("• Du Fay 有 24 部作品使用\n• 其他作曲家超過 100 部\n• 主要用於較簡單的日課聖歌：聖詩、對唱曲、詩篇、頌歌\n• cantus 中通常是 chant 的 paraphrase", {
    x: 0.5, y: 3.52, w: 4.35, h: 1.7, fontSize: 9, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2,
  });

  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 3.1, w: 4.6, h: 2.15, fill: { color: "E0EEDA" }, rounding: true });
  s.addText("🆚 Faburden vs. Fauxbourdon", { x: 5.25, y: 3.18, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.forest, fontFace: "Georgia" });
  s.addText("Faburden（英格蘭）\n  • 中聲部 = chant\n  • 上方四度 · 下方三度\n  • 即興、規則式\n\nFauxbourdon（歐陸）\n  • cantus（最高聲部）= paraphrased chant\n  • 未寫出的中聲部四度以下\n  • 寫譜作品", {
    x: 5.3, y: 3.52, w: 4.35, h: 1.7, fontSize: 9, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 2,
  });
}
...
// ── SLIDE 11 · The Polyphonic Mass ───────────────────────────────────────────
{
  ...
  s.addText("The Most Prestigious Musical Genre · 15th–16th Century", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });
  ...
  methods.forEach(([n, title, desc], i) => {
    ...
    s.addText(title, { x: 1.15, y: y + 0.08, w: 8.5, h: 0.3, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
    s.addText(desc, { x: 1.15, y: y + 0.36, w: 8.5, h: 0.3, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 12 · Cantus-firmus Mass details ────────────────────────────────────
{
  ...
  s.addText("Four Voices · Missa Caput · L'homme armé Tradition", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.12, w: 5, h: 0.04, fill: { color: C.wine } });

  // Key innovations
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 4.6, h: 3.95, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("🔑 關鍵創新 Key Innovations", { x: 0.45, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  ...
  s.addShape(pres.ShapeType.rect, { x: 5.1, y: 1.3, w: 4.6, h: 3.95, fill: { color: "F3E5C5" }, rounding: true });
  s.addText("⭐ Missa Se la face ay pale (NAWM 37)", { x: 5.25, y: 1.38, w: 4.3, h: 0.32, fontSize: 12, bold: true, color: C.wine, fontFace: "Georgia" });
  s.addText("Du Fay 1450s 寫於 Savoy\n• 首部已知以世俗歌曲為 cantus firmus 的完整彌撒\n• 以自己的 ballade tenor 為 cantus firmus\n• 可能為了紀念 1453 Savoy 公爵獲得「都靈裹屍布」——歌曲歌詞「蒼白面容、為愛受苦」可詮釋為基督受難\n\n節奏變形（augmentation）：\n• Kyrie、Sanctus、Agnus：音值加倍\n• Gloria、Credo：cantus firmus 出現三次\n  - 第 1 次：三倍音值\n  - 第 2 次：雙倍音值\n  - 第 3 次：原速——此時旋律最清楚可辨\n• 終結 Amen 處借用 ballade 的其他聲部——cantus-firmus/imitation\n• 各樂章共用 head motive——結構統一", {
...
// ── SLIDE 13 · Timeline ──────────────────────────────────────────────────────
{
  ...
  s.addText("England, Burgundy, and Music · 1400–1477", { x: 0.4, y: 0.72, w: 9.2, h: 0.38, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 1.5, y: 1.12, w: 7, h: 0.04, fill: { color: C.gold } });
  ...
  events.forEach(([date, ev], i) => {
    ...
    s.addText(date, { x: x + 0.1, y, w: 1.5, h: 0.35, fontSize: 10, bold: true, color: C.gold, fontFace: "Georgia" });
    s.addText(ev, { x: x + 1.6, y, w: 3.1, h: 0.35, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 14 · Legacy / Summary ──────────────────────────────────────────────
{
  ...
  s.addText("Chapter Summary · 持久的音樂語言", { x: 0.4, y: 0.75, w: 9.2, h: 0.35, fontSize: 13, italic: true, color: C.slate, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.12, w: 5, h: 0.04, fill: { color: C.forest } });
  ...
  summary.forEach(([icon, title, desc], i) => {
    ...
    s.addText(desc, { x: 1.1, y: y + 0.26, w: 8.5, h: 0.35, fontSize: 10, color: C.slate, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 15 · Key Terms & Further Reading ───────────────────────────────────
{
  ...
  s.addText("Key Terms · Further Reading", { x: 0.4, y: 0.75, w: 9.2, h: 0.35, fontSize: 13, color: C.sand, fontFace: "Georgia", align: "center" });
  s.addShape(pres.ShapeType.rect, { x: 2.5, y: 1.12, w: 5, h: 0.04, fill: { color: C.gold } });

  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 1.3, w: 9.4, h: 2.2, fill: { color: "2A3E2E" }, rounding: true });
  s.addText("🔑 重要術語 Key Terms", { x: 0.45, y: 1.36, w: 9.1, h: 0.3, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia" });
  ...
  termsLeft.forEach((t, i) => {
    s.addText("• " + t, { x: 0.5, y: 1.68 + i * 0.28, w: 4.5, h: 0.26, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });
  termsRight.forEach((t, i) => {
    s.addText("• " + t, { x: 5.1, y: 1.68 + i * 0.28, w: 4.5, h: 0.26, fontSize: 9, color: C.sand, fontFace: "Calibri" });
  });

  // Listening (new)
  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 3.58, w: 9.4, h: 1.55, fill: { color: "3A2015" }, rounding: true });
  s.addText("🎧 聆聽 Listen (NAWM 32–37)", { x: 0.45, y: 3.64, w: 9.1, h: 0.3, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia" });
  s.addText(
    ...
    { x: 0.5, y: 3.93, w: 9.1, h: 1.17, fontSize: 9, color: C.sand, fontFace: "Calibri" }
  );

  s.addShape(pres.ShapeType.rect, { x: 0.3, y: 5.2, w: 9.4, h: 0.5, fill: { color: "3E2030" }, rounding: true });
  s.addText("📚 Fallows, Dufay · Bent, Dunstaple · Kirkman, Cultural Life of the Early Polyphonic Mass · Strohm, Rise of European Music", {
    x: 0.5, y: 5.27, w: 9.1, h: 0.4, fontSize: 8, color: C.sand, fontFace: "Calibri", italic: true,
  });
}
