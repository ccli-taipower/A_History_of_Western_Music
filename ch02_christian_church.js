...
// ── SLIDE 3 · The Diffusion of Christianity ──────────────────────────────────
{
  ...
  facts.forEach(([label, content], i) => {
    ...
    s.addText(label, { x: 0.45, y: y + 0.07, w: 2.1, h: 0.55, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(content, { x: 2.55, y: y + 0.05, w: 6.95, h: 0.62, fontSize: 11.5, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 4 · The Judaic Heritage ────────────────────────────────────────────
{
  ...
  s.addText(bullets.map((b, i) => ({
    text: b, options: { bullet: true, breakLine: i < bullets.length - 1, fontSize: 12, color: C.darkText, fontFace: "Calibri", paraSpaceAfter: 5 },
  ...
  terms.forEach(([term, def], i) => {
    ...
    s.addText(term, { x: 6.45, y: y + 0.04, w: 2.9, h: 0.32, fontSize: 12.5, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(def, { x: 6.45, y: y + 0.38, w: 2.9, h: 0.38, fontSize: 10.5, color: C.cream, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 5 · Music in the Early Church ──────────────────────────────────────
{
  ...
  s.addText("最早有文字記載的基督教音樂活動是演唱讚美詩與詩篇（馬太 26:30；馬可 14:26；以弗所書 5:19）\nEarliest recorded musical activity: singing hymns and psalms (Matthew 26:30; Ephesians 5:19)", {
    x: 0.4, y: 0.84, w: 9.2, h: 0.55, fontSize: 12, color: C.slate, italic: true, fontFace: "Calibri",
  });

  const cards = [
    ["🏛\n會堂", "Basilica", "四世紀起，公開禮拜在大型長方形教堂（會堂式）舉行\n吟誦詩篇更有助於聲音傳播", "Large basilica buildings\nchanting carries clearly"],
    ["🙏\n修道院", "Monasteries", "修士每日數次吟誦詩篇，是靈性操練的核心\nMonastic life centered on regular psalm singing many times daily", "Psalm singing\nas spiritual discipline"],
    ["📖\n教父", "Church Fathers", "聖巴西流、聖奧古斯丁等以希臘哲學觀詮釋音樂\nChurch Fathers interpreted music using Greek philosophy", "Music shapes soul\nbut pleasure is suspect"],
    ["🚫\n樂器", "Instruments", "早期教會禁用器樂——音樂只服侍語言，器樂沒有文字\nInstruments banned: music must serve words; instruments lack text", "Unaccompanied singing\nfor over a millennium"],
  ];
  ...
  cards.forEach(([icon, name, zh, en], i) => {
    ...
    s.addText(name, { x: x + 0.1, y: 2.28, w: 1.95, h: 0.38, fontSize: 13, bold: true, color: C.gold, align: "center", fontFace: "Georgia" });
    s.addShape(pres.ShapeType.rect, { x: x + 0.3, y: 2.68, w: 1.55, h: 0.03, fill: { color: C.gold } });
    s.addText(zh, { x: x + 0.08, y: 2.74, w: 1.99, h: 1.3, fontSize: 10.5, color: C.cream, align: "center", fontFace: "Calibri" });
    s.addText(en, { x: x + 0.08, y: 4.08, w: 1.99, h: 1.0, fontSize: 10, color: C.sand, align: "center", italic: true, fontFace: "Calibri" });
  });
}
...
// ── SLIDE 6 · Church Fathers on Music ────────────────────────────────────────
{
  ...
  s.addText("「聖靈見人心傾向罪惡，便將教義的甘甜與旋律融合，以使人在不知不覺中受到教化。」\n\nThe Holy Spirit blended the delight of melody with doctrine, so that through the pleasantness of what is heard, we might unawares receive what is useful.\n\n讚美詩篇演唱：培育靈魂、凝聚社群、以音樂驅逐怒氣\nPsalm-singing: educates the soul, builds community, calms anger", {
    x: 0.45, y: 2.02, w: 4.2, h: 3.1, fontSize: 11.5, color: C.sand, fontFace: "Calibri", italic: false,
  });
  ...
  s.addText("「我在歌聲中所獲的感動……與歌詞的含意俱來；我承認這種做法是有益的。」\n\nI acknowledge the great benefit of singing in church, while wavering between the peril of pleasure and the benefit of my experience.\n\n態度：音樂有益於靈性，但若旋律比歌詞更動人，則視為罪\nTension: music aids devotion, but if the song moves more than the text, it is sinful", {
    x: 5.35, y: 2.02, w: 4.2, h: 3.1, fontSize: 11.5, color: C.sand, fontFace: "Calibri",
  });
}
...
// ── SLIDE 7 · Divisions in the Church & Dialects of Chant ───────────────────
{
  ...
  s.addText("Divisions in the Church and Dialects of Chant", {
    x: 0.4, y: 0.7, w: 9.2, h: 0.3, fontSize: 13, color: C.midBrown, italic: true, fontFace: "Calibri", margin: 0,
  });
  ...
  traditions.forEach(({ title, color, items }, col) => {
    ...
    s.addText(items.map((t, i) => ({
      text: t, options: { bullet: !t.startsWith("•"), breakLine: i < items.length - 1, fontSize: 11, color: C.cream, fontFace: "Calibri", paraSpaceAfter: 3 },
  ...
  s.addText("1054 CE：東西教會正式分裂 → 羅馬天主教 vs 東正教（拜占庭）\n1054 CE: Final split — Roman Catholic Church vs. Eastern Orthodox (Byzantine) Church", {
    x: 0.3, y: 5.05, w: 9.4, h: 0.42, fontSize: 10.5, color: C.midBrown, italic: true, fontFace: "Calibri", align: "center",
  });
}
...
// ── SLIDE 8 · The Creation of Gregorian Chant ────────────────────────────────
{
  ...
  steps.forEach(([label, text], i) => {
    ...
    s.addText(label, { x: 0.45, y: y + 0.06, w: 2.6, h: 0.75, fontSize: 11.5, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addShape(pres.ShapeType.rect, { x: 3.15, y: y + 0.12, w: 0.04, h: 0.65, fill: { color: C.gold } });
    s.addText(text, { x: 3.28, y: y + 0.08, w: 6.3, h: 0.72, fontSize: 12, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 9 · Timeline ────────────────────────────────────────────────────────
{
  ...
  events.forEach(([date, event], i) => {
    ...
    s.addText(date, { x: 0.1, y, w: 2.28, h: 0.33, fontSize: 9.5, color: C.sand, fontFace: "Calibri", align: "right", margin: 0 });
    s.addText(event, { x: 2.95, y, w: 6.8, h: 0.33, fontSize: 10.5, color: C.lightText, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 10 · The Development of Notation (Neumes) ─────────────────────────
{
  ...
  s.addText("「除非被記住，否則聲音就消逝了；它們無法寫下。」—— 塞維利亞的伊西多爾 (ca. 560–636)\nUnless sounds are remembered by man, they perish, for they cannot be written down. — Isidore of Seville", {
    x: 0.4, y: 0.84, w: 9.2, h: 0.5, fontSize: 12, color: C.slate, italic: true, fontFace: "Calibri",
  });
  ...
  stages.forEach(([title, desc], i) => {
    ...
    s.addText(title, { x: 0.45, y: y + 0.06, w: 2.4, h: 0.72, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addShape(pres.ShapeType.rect, { x: 2.95, y: y + 0.12, w: 0.04, h: 0.6, fill: { color: C.gold } });
    s.addText(desc, { x: 3.08, y: y + 0.07, w: 6.5, h: 0.72, fontSize: 11.5, color: C.cream, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 11 · Boethius & Three Types of Music ───────────────────────────────
{
  ...
  s.addText("波愛修斯生於羅馬貴族，後受哥德統治者狄奧多里克任命，依據希臘文獻（畢達哥拉斯、托勒密）撰寫《音樂基礎》。此書在中世紀被奉為最高音樂理論權威超過一千年。\nBoethius, born into Roman nobility, compiled De institutione musica from Greek sources (Pythagoras, Ptolemy). It was the authoritative text on music theory for over a thousand years.", {
    x: 0.4, y: 1.18, w: 9.2, h: 0.75, fontSize: 12, color: C.sand, fontFace: "Calibri", italic: false,
  });
  ...
  const types = [
    {
      name: "Musica Mundana\n宇宙音樂",
      desc: "天體運行、四季更替、元素和諧所產生的數學比例\nThe numerical ratios controlling movement of stars and planets, changing of seasons, and the elements",
      color: "1A3A5C",
      icon: "🌌",
    },
    {
      name: "Musica Humana\n人體音樂",
      desc: "人體與靈魂、身體各部分之間的和諧\nHarmonizes the soul and body and their parts; the mathematical harmony of the human body and soul",
      color: "2A5C1A",
      icon: "🧬",
    },
    {
      name: "Musica Instrumentalis\n樂器音樂",
      desc: "人聲或樂器產生的可聽音樂——最低層，但體現了相同的數學比例\nAudible music produced by instruments or voices; exemplifies the same principles of numerical ratios",
      color: "5C2A1A",
      icon: "🎵",
    },
  ];
  ...
  types.forEach(({ name, desc, color, icon }, i) => {
    const x = 0.3 + i * 3.25;
    s.addShape(pres.ShapeType.rect, { x, y: 2.05, w: 3.05, h: 3.15, fill: { color }, rounding: true });
    s.addText(icon, { x, y: 2.12, w: 3.05, h: 0.5, fontSize: 24, align: "center" });
    s.addText(name, { x: x + 0.1, y: 2.65, w: 2.85, h: 0.65, fontSize: 13, bold: true, color: C.gold, fontFace: "Georgia", align: "center" });
    s.addShape(pres.ShapeType.rect, { x: x + 0.3, y: 3.32, w: 2.45, h: 0.03, fill: { color: C.gold } });
    s.addText(desc, { x: x + 0.1, y: 3.4, w: 2.85, h: 1.68, fontSize: 11, color: C.sand, fontFace: "Calibri", align: "center" });
  });

  s.addText("🔑 對波愛修斯而言，音樂首先是知識的對象，而非實踐技藝。真正的音樂家是「理解」音樂的人，而非演奏者。\nFor Boethius, music was an object of knowledge, not a practical skill. The true musician understands music; the performer plays by instinct.", {
    x: 0.4, y: 5.22, w: 9.2, h: 0.22, fontSize: 10, color: C.sand, italic: true, fontFace: "Calibri",
  });
}
...
// ── SLIDE 12 · The Church Modes ──────────────────────────────────────────────
{
  ...
  const hWidths = [1.5, 1.3, 1.4, 3.0, 2.0];
  let hx = 0.35;
  headers.forEach((h, i) => {
    s.addShape(pres.ShapeType.rect, { x: hx, y: 1.12, w: hWidths[i] - 0.05, h: 0.4, fill: { color: C.wine }, rounding: false });
    s.addText(h, { x: hx + 0.05, y: 1.14, w: hWidths[i] - 0.1, h: 0.36, fontSize: 11, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    hx += hWidths[i];
  });
  ...
  modes.forEach(([mode, final, type, intervals, greek], i) => {
    ...
    let rx = 0.35;
    row.forEach((cell, j) => {
      s.addShape(pres.ShapeType.rect, { x: rx, y, w: hWidths[j] - 0.05, h: 0.42, fill: { color: bg } });
      s.addText(cell, {
        x: rx + 0.05, y: y + 0.02, w: hWidths[j] - 0.1, h: 0.38,
        fontSize: j === 0 ? 11.5 : 10.5,
        bold: j === 0,
  ...
  s.addText("🔑 每種調式依「最終音」（final）及音域範圍區別；正格（authentic）音域從 final 上行一個八度；變格（plagal）音域跨越 final 上下", {
    x: 0.35, y: 5.28, w: 9.3, h: 0.2, fontSize: 9.5, color: C.midBrown, italic: true, fontFace: "Calibri",
  });
}
...
// ── SLIDE 13 · Solmization & Guido of Arezzo ─────────────────────────────────
{
  ...
  s.addText("桂多從讚美詩《Ut queant laxis》（聖若望讚美詩）每句開頭音節命名六個音：\nGuido named six syllables from the opening notes of the hymn Ut queant laxis:", {
    x: 0.45, y: 1.74, w: 4.8, h: 0.55, fontSize: 11.5, color: C.sand, fontFace: "Calibri",
  });
  ...
  syllables.forEach(([syl, note], i) => {
    ...
    s.addText(note, { x: sx, y: 2.8, w: 0.68, h: 0.38, fontSize: 10, color: C.cream, align: "center", fontFace: "Calibri" });
  });

  s.addText("唱名法的功能：幫助歌手辨識音程（全音/半音）\n• mi → fa 之間是半音（semitone）\n• 其他相鄰音都是全音（whole tone）\n• 這使歌手能正確演唱任何聖詠旋律\n\nSolmization helped singers identify intervals: the step mi–fa is always a semitone; all others are whole tones.", {
    x: 0.45, y: 3.32, w: 4.8, h: 1.8, fontSize: 11.5, color: C.sand, fontFace: "Calibri",
  });
  ...
  const guido = [
    ["🖊 四線譜", "發明現代五線譜前身：四線有色譜（F 線紅色，C 線黃色）\nFour-line staff with colored F and C clef lines"],
    ["🔡 字母譜號", "制訂 A–G 音符字母系統（沿用至今）\nDeveloped A–G letter names for notes"],
    ["✋ 桂多手勢", "以左手關節代表音符——視覺化教學工具\nGuidonian hand: mnemonic device using finger joints"],
    ["⚡ 效率", "「一年內培養出完美的歌手」——以前需十年\nCould train singers in one year vs. ten previously"],
  ];
  guido.forEach(([label, desc], i) => {
    const y = 1.95 + i * 0.8;
    s.addText(label, { x: 5.75, y, w: 3.8, h: 0.3, fontSize: 12, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(desc, { x: 5.75, y: y + 0.3, w: 3.8, h: 0.42, fontSize: 10.5, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 14 · Chapter Summary / Echoes of History ──────────────────────────
{
  ...
  points.forEach(([icon, text], i) => {
    ...
    s.addText(text, { x: 1.05, y: y + 0.05, w: 8.5, h: 0.58, fontSize: 11.5, color: C.sand, fontFace: "Calibri", margin: 0 });
  });
}
...
// ── SLIDE 15 · Further Reading ────────────────────────────────────────────────
{
  ...
  const listening = [
    "Gregorian — Viderunt omnes  youtu.be/EN73kO2_PZA",
    "Hildegard — O Virtus Sapientiae (Sequentia)  youtu.be/77M2pkkdH10",
    "Byzantine — Cherubic Hymn (Simonopetra)  youtu.be/GhnZY9qfi4s",
    "Ambrosian — Lucernario (Vianini)  youtu.be/vapN-nybyro",
    "Guido of Arezzo — Ut queant laxis  youtu.be/ghYLCqHimfc",
  ];
  s.addText(listening.map((l, i) => ({
    text: l, options: { bullet: true, breakLine: i < listening.length - 1, fontSize: 11, color: C.cream, fontFace: "Calibri", paraSpaceAfter: 4 },
  ...
  const reading = [
    "Wikipedia: Gregorian chant — en.wikipedia.org",
    "Stanford Encyclopedia: Medieval Music Theory",
    "CPDL (Choral Public Domain Library): chant scores",
    "Boethius, De institutione musica — Open Library",
    "Guido of Arezzo, Micrologus — translated excerpts",
  ];
  s.addText(reading.map((r, i) => ({
    text: r, options: { bullet: true, breakLine: i < reading.length - 1, fontSize: 11, color: C.cream, fontFace: "Calibri", paraSpaceAfter: 4 },
  ...
  const terms = "Psalm 詩篇 · Cantillation 吟誦 · Gregorian Chant 格里高利聖詠 · Plainchant/Plainsong 平唱 · Rite 儀式 · Liturgy 禮拜儀式 · Neumes 紐姆符號 · Mode 調式 · Final 最終音 · Authentic 正格 · Plagal 變格 · Solmization 唱名法 · Musica mundana / humana / instrumentalis";
  s.addText(terms, {
    x: 0.6, y: 4.18, w: 8.8, h: 1.02, fontSize: 12, color: C.cream, fontFace: "Calibri",
  });
}
