...
// ── SLIDE 3 · European Society 800–1300 ──────────────────────────────────────
{
  ...
  s.addText("① Byzantine Empire 拜占庭：保存希臘羅馬科學與文化；多數古希臘著作因拜占庭抄寫員而倖存\n② Arab world 阿拉伯世界：最強盛的一支；延伸希臘哲學與科學；在醫學、化學、數學上貢獻卓著\n③ Western Europe 西歐：最弱最窮；800 年查理曼加冕為羅馬皇帝，宣告與拜占庭獨立\n    Western Europe was weakest; Charlemagne's coronation in 800 asserted independence from Byzantium", {
    x: 0.5, y: 1.72, w: 9.0, h: 1.1, fontSize: 14, color: C.sand, fontFace: "Calibri", valign: "top",
  });
  ...
  s.addText(dev.map((t, i) => ({
    text: t, options: { bullet: false, breakLine: i < dev.length - 1, fontSize: 14, color: C.sand, fontFace: "Calibri", paraSpaceAfter: 1 }, valign: "top",
...
// ── SLIDE 4 · Latin and Vernacular Song ──────────────────────────────────────
{
  ...
  s.addText("大多失傳：90% 人口為不識字的鄉民\nMost lost — 90% nonliterate\n\n倖存的主要類型：\n\n• Epic 史詩：以旋律公式吟唱\n  Chanson de geste（功業之歌）\n  最著名：《羅蘭之歌》(ca. 1100)\n  Beowulf、Nibelungs、Eddas\n\n• 市井街頭叫賣、民歌、搖籃曲\n  僅因被複音音樂引用而留存\n  Preserved like flies in amber", {
...
// ── SLIDE 5 · Minstrels & Professional Musicians ─────────────────────────────
{
  ...
  cats.forEach(([name, tag, desc], i) => {
    ...
    s.addText(name, { x: 0.45, y: y + 0.08, w: 2.8, h: 0.35, fontSize: 14, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(tag, { x: 0.45, y: y + 0.42, w: 2.8, h: 0.3, fontSize: 14, color: C.sand, italic: true, fontFace: "Calibri", valign: "top", margin: 0 });
...
// ── SLIDE 7 · Fin' Amors — The Poetry ────────────────────────────────────────
{
  ...
  s.addText("• 非雙方對等的愛，而是形式化、理想化的愛——情人藉之自我精鍊\n  Not mutual; formal, idealized love through which the lover is refined\n• 對象通常是另一貴族之妻；從遠方、帶著謹慎、尊敬、謙卑地愛慕\n  Object usually another man's wife; adored from a distance with discretion, respect, humility\n• 愛慕語言常與對聖母的崇拜相近——女士被塑造成高不可攀、無法回應的理想\n  The language borders on devotional songs to the Virgin Mary\n• 「courtly love」一詞為 19 世紀後創；不是事實而是藝術形式，向男性同儕展現教養", {
    x: 0.5, y: 1.72, w: 9.0, h: 1.3, fontSize: 14, color: C.sand, fontFace: "Calibri", valign: "top",
  });
  ...
  genres.forEach(([name, desc], i) => {
    ...
    s.addText(name, { x, y, w: 1.8, h: 0.35, fontSize: 14, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(desc, { x: x + 1.85, y, w: 2.6, h: 0.35, fontSize: 14, color: C.sand, fontFace: "Calibri", valign: "top", margin: 0 });
  });
}
...
// ── SLIDE 9 · Songs in Other Lands ───────────────────────────────────────────
{
  ...
  lands.forEach(([name, tag, desc], i) => {
    ...
    s.addText(name, { x: 0.45, y: y + 0.08, w: 3.1, h: 0.35, fontSize: 14, bold: true, color: C.gold, fontFace: "Georgia", margin: 0 });
    s.addText(tag, { x: 0.45, y: y + 0.42, w: 3.1, h: 0.35, fontSize: 14, color: C.sand, italic: true, fontFace: "Calibri", valign: "top", margin: 0 });
    s.addShape(pres.ShapeType.rect, { x: 3.6, y: y + 0.1, w: 0.03, h: 0.65, fill: { color: C.gold } });
    s.addText(desc, { x: 3.7, y: y + 0.08, w: 5.95, h: 0.75, fontSize: 14, color: C.cream, fontFace: "Calibri", valign: "top", margin: 0 });
  });
...
// ── SLIDE 10 · Medieval Instruments ──────────────────────────────────────────
{
  ...
  groups.forEach(([title, color, items], i) => {
    ...
    s.addText(title, { x: x + 0.15, y: 1.23, w: 4.3, h: 0.35, fontSize: 14, bold: true, color: C.gold, fontFace: "Georgia" });
    s.addShape(pres.ShapeType.rect, { x: x + 0.25, y: 1.62, w: 4.1, h: 0.02, fill: { color: C.gold } });
    s.addText(items.map((t, j) => ({
      text: t, options: { bullet: true, breakLine: j < items.length - 1, fontSize: 14, color: C.cream, fontFace: "Calibri", paraSpaceAfter: 3 }, valign: "top",
...
// ── SLIDE 11 · Dance Music ───────────────────────────────────────────────────
{
  ...
  s.addText("12–14 世紀法國最流行的社交舞 · 圓圈舞蹈 · 通常由其中一位或多位舞者歌唱伴奏\nThe most popular social dance in France, 12th–14th c.; circle dance usually led by singing dancers\n• 可加入樂師伴奏（如《玫瑰傳奇》中所描述）\n• 儘管流行，現存旋律僅約 24 首——多靠口傳心授，極少寫下", {
    x: 0.5, y: 1.72, w: 9.0, h: 1.25, fontSize: 14, color: C.sand, fontFace: "Calibri", valign: "top",
  });
...
// ── SLIDE 14 · The Lover's Complaint (Continuing Presence) ───────────────────
{
  ...
  s.addText("中世紀歌曲的共同特徵（延續至今的西方歌曲原型）", {
  ...
  s.addText("• 分節歌式（strophic）· 自然音階（diatonic）· 主要音節式（syllabic）\n  Strophic · diatonic · primarily syllabic\n• 主要級進，音域約一個八度 · 清楚的音高中心\n  Mostly stepwise, range ~an octave, clear pitch center\n• 樂句短且大致等長，弧形起伏，回落至終止\n  Short musical phrases, roughly equal in size, arch-shaped\n• 常用副歌（refrain）· 主題多為「未能如願的純潔之愛」\n  Refrains common · theme of pure, unattainable love", {
    x: 0.5, y: 1.62, w: 9.0, h: 1.42, fontSize: 14, color: C.cream, fontFace: "Calibri", valign: "top",
  });
...
// ── SLIDE 16 · Further Reading ───────────────────────────────────────────────
{
  ...
  const readingLeft = [
    "Paden — Troubadour Poems",
    "Rosenberg — Songs of the Trouvères",
    "Stevens — Words and Music",
  ];
  const readingRight = [
    "Page — The Owl and the Nightingale",
    "Wikipedia: Troubadour / Cantigas",
  ...
  const terms = "Troubadour / Trobairitz · Trouvère · Jongleur · Minstrel · Chanson de geste · Fin' amors · Canso · AAB form · Bar Form · Minnesinger · Cantiga · Vielle · Shawm · Estampie · Istampita · Rondeau";
  s.addText(terms, {
